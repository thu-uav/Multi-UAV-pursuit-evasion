...
def evaluate(params, real_data, sim, drone, controller):
    ...
    # pos_error = torch.zeros(size=shuffled_idx.shape, dtype=torch.float)
    # quat_error = torch.zeros(size=shuffled_idx.shape, dtype=torch.float)
    # vel_error = torch.zeros(size=shuffled_idx.shape, dtype=torch.float)
    # omega_error = torch.zeros(size=shuffled_idx.shape, dtype=torch.float)
...
def main(cfg):
    ...
    for epoch in range(500):
        ...
        suggested_para = np.array(opt.ask())
        grad, pos_error, quat_error, vel_error, omega_error = func(suggested_para, real_data, sim, drone, controller)
