...
omega_x = np.array(df['omega.r'])[start_idx:clip_idx]
omega_y = np.array(df['omega.p'])[start_idx:clip_idx]
omega_z = np.array(df['omega.y'])[start_idx:clip_idx]
...
axs[0].plot(timestamps, omega_x, label='omega_x', color='red')
axs[0].plot(timestamps, target_rate_r, label='target_roll_rate', color='green')
axs[0].legend()
axs[0].grid(True)

axs[1].plot(timestamps, real_rate_p, label='real_pitch_rate', color='blue')
axs[1].plot(timestamps, target_rate_p, label='target_pitch_rate', color='green')
axs[1].plot(timestamps, omega_y, label='omega_y', color='red')
axs[1].legend()
axs[1].grid(True)

axs[2].plot(timestamps, real_rate_y, label='real_yaw_rate', color='blue')
axs[2].plot(timestamps, omega_z, label='omega_z', color='red')
axs[2].plot(timestamps, target_rate_y, label='target_yaw_rate', color='green')
axs[2].legend()
axs[2].grid(True)

axs[3].plot(timestamps, real_thrust, label='real_thrust', color='blue')
axs[3].plot(timestamps, target_thrust, label='target_thrust', color='green')
axs[3].legend()
axs[3].grid(True)
