...
class FormationBallForward(IsaacEnv):
        ...
        self.observation_spec = CompositeSpec({
            "agents": {
                "observation": CompositeSpec({
                    "obs_self": UnboundedContinuousTensorSpec((1, obs_self_dim+3)), # (23+3) +3
    ...
    def _pre_sim_step(self, tensordict: TensorDictBase):
        ...
        #self.flag.bitwise_or_(flag)
            ...
            ball_radius = torch.rand(len(should_throw)) * 5 # [0, 5]
            ball_pos[:,0] = ball_radius * torch.cos(ball_ang)
            ball_pos[:,1] = ball_radius * torch.sin(ball_ang)
            ball_pos[:, :2] += self.middle_xy
            # height of the ball 2-4 m
            ball_pos[:,2] = torch.rand(len(should_throw), device=self.device) * 2 + 2
            ...
            # target_ball_pos
            target_ball_pos = torch.zeros(centre_D.shape, device=self.device)
            target_ball_pos[:, 0] = torch.rand(1, device=self.device)*4-2 + drone_x_pos
            target_ball_pos[:, 1] = torch.rand(1, device=self.device)*2-1 # [-1,1]
    ...
    def _compute_reward_and_done(self):
        ...
        if should_penalise.any():
            should_penalise = torch.nonzero(should_penalise).squeeze(-1)
