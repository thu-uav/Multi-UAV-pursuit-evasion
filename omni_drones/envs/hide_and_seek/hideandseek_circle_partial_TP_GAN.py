...
def select_unoccupied_positions_withcylinders(occupancy_matrix, cylinder_grid, active_cylinders_list, num_drones, num_target):
    ...
    # occupancy_matrix: add cylidners occupancy
    
    return torch.stack(all_chosen_coords), occupancy_matrix

def select_unoccupied_positions_GAN(occupancy_matrix, candidate_cylinder_grid, num_drones, num_target):
    ...
    for i in range(batch_size):
        ...
        if len(flat_indices) > 0:
            occupancy_matrix[i][flat_indices[:, 0], flat_indices[:, 1]] = 1
        # choose real cylinders, no duplication, not out of the circle
        ...
        chosen_coords = available_coords[torch.randperm(available_coords.size(0))[:n]]
        objects_coords = torch.concat([candidate_cylinder_grid[i], chosen_coords])
        all_chosen_coords.append(objects_coords)
...
class HideAndSeek_circle_partial_TP_GAN(IsaacEnv): 
    ...
    def rejection_sampling_for_init(self, cylinders_pos, env_ids: torch.Tensor):
        ...
        objects_grid, _ = select_unoccupied_positions_withcylinders(grid_map, cylinders_grid, self.active_cylinders, num_drones=self.num_agents, num_target=1)
